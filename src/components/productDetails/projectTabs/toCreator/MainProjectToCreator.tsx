...
export default function MainProjectToCreator({}: Props) {
    const { ProjectInfo ,UserInfo } = useProjectDetails();
    ...
    return (
        <section className="flex flex-col items-center justify-center w-full gap-y-[3vh] ">
            ...
            {!currentUserStatus.isLoading && UserInfo.receivedCommentIds && UserInfo.receivedCommentIds.length > 0 && 
                <div className="flex flex-col w-[70vw] items-center">
                    {
                        UserInfo.receivedCommentIds.reverse().map((e) => {
                            
                            return <p>
                                asdfg
                            </p>
                            // return    <CommentSection key={e.authorId} data={e} type="to-creator"/>
                        })
                    }
                </div>
            }
